				    ...
				    std::vector<std::unique_ptr<FrameBuffer>> *buffers)
{
	for (unsigned i = 0; i < count; ++i) {
		std::unique_ptr<FrameBuffer> buffer = gbm_bo_create(gbm, w, h, f, GBM_BO_USE_RENDERING); /*confirm width,height,format*/
		buffers->push_back(std::move(buffer));
	}
}

int SimpleConverter::queueBuffer(FrameBuffer *input)
{
